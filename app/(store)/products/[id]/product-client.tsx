    ...
    <div className="flex">
      <div className="w-1/5 flex flex-col space-y-2 overflow-auto items-center">
        {images?.map((item, idx, arr) => (
          <div key={item.storagePath} className="p-2 w-1/2 hover:border ">
            <img
              src={item.url}
              alt="product image"
...
export const ProductCartButton = () => {
  ...
  const addToCartHandler = () => {
    ...
    //   variant.attributes?.every(({ value, name }) => selected[name] === value),
    // )
    throw new Error('Find variant is not implemented')
  ...
  return (
    <>
      <div className="flex gap-2 relative border-b mt-2">
        <p className="p-2 text-blue-700 text-sm font-semibold">{displayText}</p>
