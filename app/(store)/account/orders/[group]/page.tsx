...
import { IOrderStatus, IOrder } from 'types'
...
const getOrderByGroup = async (group: string) => {
  ...
  const statusList = (await statusListResp.json()) as IOrderStatus[]
  ...
  const response = await springDataProvider.custom({
    ...
    query: {
      uid: (() => {
        throw new Error('UID is missing here')
      })(),
      st: statusParam,
    },
    meta: {
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} className="border-t">
                <td className="px-4 py-4 text-blue-500">{order.id}</td>
                <td className="px-4 py-4">{order.purchasedAt}</td>
